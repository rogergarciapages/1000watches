...
export default function PhotoGallery({ watchId, userId, onPhotoCountChange }: PhotoGalleryProps) {
  ...
  if (photos.length === 0) return (
    <div className="text-center py-8 font-sans">
      <p className="text-[var(--text-muted)] mb-2">No photos yet</p>
      <p className="text-[var(--text-dim)] text-sm">Be the first to add a photo!</p>
    </div>
  )
  ...
  return (
    <div className="space-y-4 font-sans">
      {/* Main photo display */}
      <div className="relative aspect-square rounded-2xl overflow-hidden bg-[var(--bg-secondary)] border border-[var(--border-medium)] shadow-md">
        <img
        ...
        {displayed.uploader_username && (
          <div className="absolute top-3 left-3">
            <span className="px-2.5 py-1 rounded-full bg-[var(--bg-elevated)]/80 backdrop-blur-sm border border-[var(--border-medium)] text-[var(--text-secondary)] text-xs">
        ...
        {displayed.is_default && (
          <div className="absolute top-3 right-3">
            <span className="px-2.5 py-1 rounded-full bg-amber-500 text-black text-xs font-bold shadow-md">
              Featured
            </span>
          </div>
        )}
      ...
      <div className="grid grid-cols-4 md:grid-cols-6 gap-2">
        {photos.map(photo => (
          ...
          >
            ...
            <div className="absolute bottom-0 right-0 px-1 py-0.5 bg-black/70 text-white text-[8px] flex items-center gap-0.5">
...
function PhotoRow({ photo, hasVoted, isLoading, isDeleting, canDelete, onVote, onDelete }: {
  ...
  return (
    <div className="flex items-center gap-3 p-3 rounded-lg bg-[var(--bg-card)] border border-[var(--border-medium)]">
      <button onClick={() => {}} className="shrink-0">
        {isDeleting ? (
          <div className="w-16 h-16 rounded-lg bg-[var(--bg-secondary)] flex items-center justify-center">
      ...
      <div className="flex-grow min-w-0 font-sans">
        <p className="text-sm text-[var(--text-primary)] truncate font-medium">{photo.caption || 'No caption'}</p>
        <p className="text-xs text-[var(--text-muted)] flex items-center gap-2">
          {photo.uploader_username && <span>by {photo.uploader_username}</span>}
          {photo.is_default && <span className="text-amber-500 font-bold">Featured</span>}
      ...
      <div className="flex items-center gap-2 shrink-0 font-sans">
        <button
          ...
          className={`flex items-center gap-1 px-3 py-1.5 rounded-lg border text-sm transition-all ${
            hasVoted
              ...
              : 'bg-[var(--bg-primary)] border-[var(--border-medium)] text-[var(--text-secondary)] hover:bg-amber-500/10 hover:border-amber-500/40 hover:text-amber-500'
          }`}
        >
          {isLoading ? (
            <div className="w-4 h-4 border-2 border-amber-500/20 border-t-amber-500 rounded-full animate-spin" />
        ...
        {canDelete && (
          <button
            onClick={onDelete}
            disabled={isLoading || isDeleting}
            className="flex items-center gap-1 px-3 py-1.5 rounded-lg border border-[var(--border-medium)] text-[var(--text-muted)] hover:border-red-500/50 hover:text-red-500 text-sm transition-all"
            title="Delete photo"
          >
            {isDeleting ? (
      ...
      <button
        ...
        className={`flex items-center gap-2 px-4 py-2 rounded-xl border transition-all ${
          hasVoted
            ...
            : 'bg-[var(--bg-elevated)]/80 backdrop-blur-sm border-[var(--border-medium)] text-[var(--text-primary)] hover:bg-amber-500/20 hover:border-amber-500/50 hover:text-amber-500'
        }`}
      >
        {isLoading ? (
          <div className="w-4 h-4 border-2 border-amber-500/20 border-t-amber-500 rounded-full animate-spin" />
        ) : (
          <svg className={`w-5 h-5 ${hasVoted ? 'fill-amber-500 text-amber-500' : 'fill-none stroke-current'}`} viewBox="0 0 24 24" strokeWidth={2}>
            <path d="M12 4l-8 8h5v8h6v-8h5z" />
          </svg>
        )}
